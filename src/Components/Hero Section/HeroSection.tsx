...
export default function HeroSection() {
    const HoverEffect = 'w-[185px] h-[62px] bg-transparent border border-brand-hover flex items-center justify-center text-brand-hover  hover:before:bg-brand-hover relative overflow-hidden border   px-3 text-white shadow-2xl transition-all before:absolute before:bottom-0 before:left-0 before:top-0 before:z-0 before:h-full before:w-0 before:bg-brand-hover before:transition-all before:duration-500 hover:text-hover hover:shadow-brand-hover hover:before:left-0 hover:before:w-full'
    return(
        <>
                ...
                {heroData.map((data)=>(
                    <div className="w-full h-[415px] flex flex-col  text-lg uppercase relative right-140 ">
                        ...
                        <div className="w-full uppercase h-[110px] items-start flex gap-5 ">
                            <a href="#" className="w-[227px] h-[62px] font-sahel bg-brand-hover text-center items-center justify-center flex font-extrabold text-hover text-lg leading-5 hover:bg-[#3d5229] duration-500 transition-all hover:duration-500 hover:text-brand-hover">Shop the drop</a>
                            <a href="#" className={HoverEffect}><span className="relative z-10 font-sahel">view film</span></a>
                        </div>
                    </div>
                ))}
            </section>
